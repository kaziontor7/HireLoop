...
export default function NewJobForm({company}: {company: any }) {
    ...
    const [workplace, setWorkplace] = useState("onsite");
     ...
     const companyData = Array.isArray(company) && company?.[0] ? company[0] : null ;
    ...
    // If no company data is available, prompt the user to add a company first
    if (!companyData) {
        return (
            <div className="w-full min-h-screen p-4 md:p-8 flex items-center justify-center">
                <div className="max-w-xl w-full bg-[#111111] rounded-2xl border border-[#2a2a2a] shadow-2xl p-8 text-center">
                    <h2 className="text-xl font-semibold text-white mb-2">Add a company to post jobs</h2>
                    <p className="text-sm text-gray-400 mb-6">You need to create or select a company before you can post a job.</p>
                    <div className="flex items-center justify-center gap-4">
                        ...
                        <Button
                            type="button"
                            onPress={() => router.back()}
                            className="bg-transparent border border-gray-700 text-white px-6 py-2 rounded-xl"
                        >
                            Back
                        </Button>
                    </div>
    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
       ...
       const jobData = {
            ...
            companyId: companyData._id ,// Replace with actual company ID from session/DB
            companyLogo: companyData.logoUrl || "", // Optional: Include company logo URL if available
        };
        const res = await createJob(jobData);
        if(res.acknowledged) {
            setIsLoading(false);
            toast.success("Job created successfully!");
            router.push("/dashboard/recruiter/jobs"); // Redirect to jobs list after successful creation
        } else {
                setIsLoading(false);
                                ...
                                <div className="md:col-span-2 flex flex-col md:flex-row gap-6 p-4 rounded-xl border border-[#2a2a2a] bg-[#161616]">
                                    ...
                                    <div className="flex-1 w-full flex flex-col gap-2">
                                        ...
                                        <Select 
                                           ...
                                           onChange={(value) => setWorkplace(value as string)}
                                        >
                                            <Select.Trigger className="bg-[#1a1a1a] border border-gray-700 text-white rounded-xl h-12">
                                                <Select.Value />
                                                <Select.Indicator />
